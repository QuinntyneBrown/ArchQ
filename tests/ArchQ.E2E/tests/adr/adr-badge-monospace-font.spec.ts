...
test.describe('BUG-034: Status badges should use monospace font', () => {
  test.setTimeout(120000);

  test('status badge should use Geist Mono / monospace font', async ({ browser }) => {
    const context = await browser.newContext({ viewport: { width: 1440, height: 900 } });
    const page = await context.newPage();

    const ts = Date.now();
    const email = `badge-mono-${ts}@example.com`;

    await context.request.post('http://localhost:5299/api/auth/register', {
      data: { fullName: 'Badge Mono', email, password: 'S3cur3P@ss!', organizationName: `BadgeMono${ts}` },
      timeout: 60000,
    });
    ...
    // Create an ADR to get a badge
    await page.click('[data-testid="new-adr-button"]');
    await page.waitForURL(/\/adrs\/new/, { timeout: 5000 });
    await page.fill('[data-testid="adr-title-input"]', 'Badge Mono Test');
    ...
    const fontFamily = await badge.evaluate(el => getComputedStyle(el).fontFamily);
    const isMonospace = /mono|consolas|courier/i.test(fontFamily);
    expect(
      isMonospace,
      `Status badge font-family is "${fontFamily}" — should contain a monospace font per design`
    ).toBe(true);
